...
contract NFTCampaign is Ownable, ReentrancyGuard {
    ...
    function registerCampaign(
        ...
        require(
            IERC721(nftAddress).balanceOf(address(this)) >= 0,
            "Invalid NFT address"
        );
     ...
     */
    function acceptCampaign(uint256 campaignId)
        external
        onlyOwner
     ...
     */
    function addMoreSlots(uint256 campaignId, uint256 slots)
        external
        nonReentrant
    {
        require(
            _campaigns[campaignId].campaignOwner == _msgSender(),
    ...
    function claim(
        ...
        for (uint256 i = 0; i < nftIds.length; i++) {
            if (
                IERC721(_campaigns[campaignId].nftAddress).ownerOf(nftIds[i]) ==
                _msgSender() &&
                _claimStatus[campaignId][nftIds[i]] == false
            ) {
                if (_campaigns[campaignId].remainSlots <= 0) break;
                totalClaim = totalClaim.add(
                    _campaigns[campaignId].amountPerSlot
                );
                _claimStatus[campaignId][nftIds[i]] = true;
                _campaigns[campaignId].remainSlots = _campaigns[campaignId]
                    .remainSlots
                    .sub(1);
            }
        }
    ...
    function forceEnd(uint256 campaignId) external nonReentrant {
        ...
        uint256 remainFunds =
            _campaigns[campaignId].remainSlots.mul(
                _campaigns[campaignId].amountPerSlot
            );
     ...
     */
    function getCampaignsByOwner(address user)
        external
        view
     ...
     */
    function getCampaignsByNft(address nft)
        external
        view
        returns (uint256[] memory)
    {
     ...
     * @param nftId NFT id
     */
    function getClaimStatus(uint256 campaignId, uint256 nftId)
        external
        view
        returns (bool)
    {
